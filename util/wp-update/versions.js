// Comparing the version strings plugins and themes put in their headers. These
// aren't necessarily semver; the priority is that nothing reads as an upgrade
// unless it clearly is, since a wrong answer overwrites files with an older one.

// Pre-release tags rank below a plain release, so 3.0.0-rc.7 sorts under 3.0.0.
// An unknown word ranks lowest: if we can't place it, don't call it an upgrade.
...
// Reads one field from a WordPress file header, whether a plain comment or a
// doc block ("Version: 1.0.2" or " * Version:  1.0.2").
